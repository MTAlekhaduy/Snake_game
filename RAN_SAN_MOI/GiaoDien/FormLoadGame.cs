...
namespace RAN_SAN_MOI
{
    public partial class FormLoadGame : Form
    {
        ...
        {
            InitializeComponent();
            this.FormClosing += FormLoadGame_FormClosing;
            // Existing setup
        ...
        protected virtual void FormLoadGame_Load(object sender, EventArgs e)
        {
            ...
            // ── Kiểm tra có snapshot được truyền vào từ FormDangNhap không ──
            TrangThaiGameRecord snapshot = this.Tag as TrangThaiGameRecord;
            if (snapshot != null)
            {
                // Load lại đúng trạng thái đã lưu
                ...
                if (!string.IsNullOrEmpty(snapshot.DuongDanBackground) && File.Exists(snapshot.DuongDanBackground))
                {
                    Setting.BackgroundDangChon = new Setting.BackgroundRecord
                    {
                        TenHienThi = Path.GetFileNameWithoutExtension(snapshot.DuongDanBackground),
        ...
        private void FormLoadGame_FormClosing(object sender, FormClosingEventArgs e)
        {
